...
class Twitch
{
    public static function GetViewerCount(): int|string
    {
        $res = Http::withHeaders([
            'Authorization' => 'Bearer ' . TwitchToken::where('invalidated', '=', false)
                                                      ->first()?->access_token,
            'Client-Id'     => config('twitch.client_id'),
        ])
                   ->withQueryParameters([
                       'user_login' => 'chachamaxx',
                   ])
        ...
        if ($res->status() === 200) {
            ...
            TwitchToken::where('invalidated', '=', false)
                       ->first()
